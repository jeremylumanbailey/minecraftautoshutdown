...
var child = exec(
  "/usr/bin/java -jar /home/ec2-user/endtimespart2/mcserver/server.jar nogui",
...
function checkIfPlayersOnline(err, res) {
  if (err) {
    ...
    if (
      this.amountOfTimeWithoutPlayersOnline >= 3240000 &&
      res.players.online == 0
    ) {
      writeToChildConsole(
        "say No players detected on server. Server is shutting down."
      );
      console.log(
        "No players detected on server for extendend period of time. Server is shutting down"
      );
      writeToChildConsole("stop");
      child.stdin.end();
      rl.close();
